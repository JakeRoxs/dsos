...
您可以在GitHub的发布页面下载： https://github.com/jakeroxs/rekindled-server/releases
...
Rekindled Server使用自己的保存文件，只要你不将Rekindled Server保存文件复制回你的官方服务器保存文件，应该就没问题。
...
Rekindled Server使用自己的存档文件来避免与官方服务器保存文件的任何问题。如果你想将官方服务器的存档文件转移到Rekindled Server，请点击Loader底部的设置（齿轮）图标，然后单击复制原版存档按钮。

我们不提供将Rekindled Server存档复制回原版存档的自动化选项，以保证安全。如果你~真的~想这样做，你可以找到保存文件存储的文件夹，并将.rekindled文件重命名为.sl2。

## 我可以通过docker运行服务器吗

是的，目前为Rekindled Server发布了两个docker容器，每次发布新版本时都会自动更新：

jakeroxs/rekindled-server - 这是游戏服务器，大部分情况下你需要的是这个。
jakeroxs/rekindled-hub - 这是集线器服务器，除非你正在fork rekindled-server，否则你可能不需要这个。

如果你想快速运行服务器，可以使用这个单行命令。注意，它将Saved文件夹挂载到主机文件系统上的/opt/rekindled-server/Saved，使修改配置文件更容易。您可以访问/opt/rekindled-server/Saved查看和修改配置文件。
...
我们使用cmake生成项目文件。你可以使用cmake前端生成项目文件，或者你可以使用Tools文件夹中的generate\_\* shell脚本之一。
...
# 运行主服务器
nix run github:jakeroxs/rekindled-server#master-server
```

nix版本将配置存储在 `${XDG_CONFIG_HOME:-$HOME/.config}/rekindled-server`
...
│   ├── Loader/            简单的winforms应用程序，加载DS3以便连接到自定义Rekindled Server。
